...
class MyPredictor:
    ...
    def infer(self, test_image, conf_thresh, top_num=200):
        ...
        # print("data.data().shape is ", data.data().shape)

        shape = [np.array([608, 608], dtype=np.int32)]
        shape = np.expand_dims(shape, 0)
        print("shape.shape is ", shape.shape)
        # print("shape.data().shape is ", shape.data().shape)

        # 1. Set config information
        config = CxxConfig()
        if self._model_file != '' and self._param_file != '':
            config.set_model_file(self._model_file)
            config.set_param_file(self._param_file)
        else:
            config.set_model_dir(self._model_dir)
        places = [Place(TargetType.HUAWEI_ASCEND_NPU, PrecisionType.FP32),
                #   Place(TargetType.X86, PrecisionType.FP32),
                  Place(TargetType.Host, PrecisionType.FP32)]
        config.set_valid_places(places)

        # 2. Create PaddlePredictor
        predictor = create_paddle_predictor(config)

        # 3. Set input data
        input_image = predictor.get_input(0)
        input_image.resize([1, 3, 608, 608])
        input_image.set_float_data(data.data())
        # image.name = "data"
        # image.shape = [1, 3, 608, 608]
        # image.dtype = PaddleDType.FLOAT32
        # image.data = PaddleBuf(data.flatten().astype(np.float32).tolist())

        input_imgsize = predictor.get_input(1)
        input_imgsize.resize([1, 2])
        input_imgsize.set_int32_data(shape.data())
        ...
        inputs = [input_image, input_imgsize]
