...
def test(epoch_id, test_loader, model, cost):
    ...
    model.train()
    print('Epoch ID: {}, Top1 accurary:: {:.5f}, Top5 accurary:: {:.5f}'
          .format(epoch_id + 1,
                  np.array(avg_acc[0]).mean(), np.array(avg_acc[1]).mean()))
...
def main(args):
    ...
    for epoch_id in range(EPOCH_NUM):
        ...
        print(
            'Epoch ID: {}, Epoch time: {:.5f} s, reader_cost: {:.5f} s, batch_cost: {:.5f} s, exec_cost: {:.5f} s, average ips: {:.5f} samples/s'
            .format(epoch_id + 1, epoch_cost, reader_cost.sum, batch_cost.sum,
                    batch_cost.sum - reader_cost.sum, avg_ips))
...
def log_info(reader_cost, batch_cost, epoch_id, iter_max, iter_id):
    ...
    print(
        'Epoch [{}/{}], Iter [{:0>2d}/{}], reader_cost: {:.5f} s, batch_cost: {:.5f} s, exec_cost: {:.5f} s, ips: {:.5f} samples/s, {}'
        .format(epoch_id + 1, EPOCH_NUM, iter_id + 1, iter_max, reader_cost.avg,
                batch_cost.avg, batch_cost.avg - reader_cost.avg, BATCH_SIZE /
                batch_cost.avg, eta_msg))
