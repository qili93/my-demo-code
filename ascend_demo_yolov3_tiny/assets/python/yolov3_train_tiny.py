...
class TestYoloV3Net(fluid.dygraph.Layer):
  def __init__(self):
    ...
    self.conv1 = fluid.dygraph.Conv2D(num_channels=1, num_filters=21, filter_size=3, stride=1)
  ...
  def forward(self, input_data, img_size):
    # [-1, 1, 10, 10]
    ...
    out = fluid.layers.multiclass_nms(bboxes=concat_0_out, scores=concat_1_out,
                                      score_threshold=0.01, nms_top_k=32, keep_top_k=16, nms_threshold=0.45, 
                                      normalized=False, nms_eta=1, background_label=-1)
    return out
...
def train_yolov3(num_epochs, save_dirname):
    ...
    with fluid.dygraph.guard(place):
        ...
        input_data = np.random.random((BATCH_SIZE, 1, 10, 10)).astype('float32')
        in_imgsize = np.random.randint(10, 20, (BATCH_SIZE, 2)).astype('int32')

        input_var = fluid.dygraph.base.to_variable(input_data)
        in_imgsize_var = fluid.dygraph.to_variable(in_imgsize)

        for epoch in range(num_epochs):
            output = yolo_v3_net(input_var, in_imgsize_var)
        ...
        # save inference model
        out_dygraph, static_layer = TracedLayer.trace(yolo_v3_net, inputs=[input_var, in_imgsize_var])
...
if __name__ == '__main__':
    BATCH_SIZE = 2
    train_yolov3(num_epochs=2, save_dirname='../models/yolov3_tiny_model')