...
class TestYoloV3Net(fluid.dygraph.Layer):
  ...
  def forward(self, input_data, img_size):
    ...
    out = fluid.layers.multiclass_nms(bboxes=concat_0_out, scores=concat_1_out, 
                                      score_threshold=0.01, nms_top_k=1000, keep_top_k=100, nms_threshold=0.45, 
...
def train_yolov3(num_epochs, save_dirname):
    ...
    with fluid.dygraph.guard(place):
        ...
        input_data = np.random.random((BATCH_SIZE, 1, 28, 28)).astype('float32')
...
if __name__ == '__main__':
    BATCH_SIZE = 1
    train_yolov3(num_epochs=1, save_dirname='./test_model')