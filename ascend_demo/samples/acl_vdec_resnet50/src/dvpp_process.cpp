...
DvppProcess::DvppProcess(aclrtStream& stream) : stream_(stream),
    ...
    modelInputWidth_(0), modelInputHeight_(0)
{
}
...
void DvppProcess::SetInput(int inputWidth, int inputHeight,
                           int32_t format)
{
...
Result DvppProcess::InitOutputPara(int modelInputWidth, int modelInputHeight)
{
    ...
    int resizeOutWidthStride = (resizeOutWidth + 15) / 16 * 16;
    int resizeOutHeightStride = (resizeOutHeight + 1) / 2 * 2;
...
{
    uint32_t jpegOutWidthStride = (inputWidth_ + 15) / 16 * 16;
    uint32_t jpegOutHeightStride = (inputHeight_ + 1) / 2 * 2;
    ...
    acldvppSetPicDescData(resizeInputDesc_, picOutBufferDev_);
    acldvppSetPicDescFormat(resizeInputDesc_, static_cast<acldvppPixelFormat>(format_));
...
Result DvppProcess::InitResizeOutputDesc()
{
    ...
    int resizeOutputWidthStride = (modelInputWidth_+ 15) / 16 * 16;
    int resizeOutputHeightStride = (modelInputHeight_ + 1) / 2 * 2;
    ...
    acldvppSetPicDescFormat(resizeOutputDesc_, static_cast<acldvppPixelFormat>(format_));
