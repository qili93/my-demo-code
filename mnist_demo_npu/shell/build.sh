...
function print_usage {
    ...
    echo -e "   ./build.sh --arm_abi=<abi> --with_log=<ON|OFF> tiny_publish"
    echo
    echo -e "compile full publish so lib:"
    echo -e "   ./build.sh --arm_abi=<abi> --with_log=<ON|OFF> full_publish"
    echo
    ...
    for i in "$@"; do
        case $i in
            ...
            --with_log=*)
                ...
                ;;
            tiny_publish)
                make_publish $ARM_ABI $WITH_LOG tiny
                shift
                ;;
            full_publish)
                make_publish $ARM_ABI $WITH_LOG full
                shift
                ;;
            *)
                # unknown option
