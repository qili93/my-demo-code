...
std::shared_ptr<Predictor> InitPredictor() {
  ...
  // Enable custom_cpu
  config.EnableCustomDevice("custom_cpu", 0);
...
void run(Predictor *predictor, const std::vector<float> &input,
  ...
  LOG(INFO) << "run avg time is " << time_diff(st, time()) / FLAGS_repeats << " ms";
}

int main(int argc, char *argv[]) {
  google::ParseCommandLineFlags(&argc, &argv, true);
  auto predictor = InitPredictor();
  std::vector<int> input_shape = {FLAGS_batch_size, 1, 28, 28};
  std::vector<float> input_data(FLAGS_batch_size * 1 * 28 * 28);
