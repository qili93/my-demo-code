...
IMAGE_CROP_PATH="../assets/images/face-crop.jpg"
IMAGE_DATA_PATH="../assets/images/face.raw"
IMAGE_MARK_PATH="../assets/images/face-mark.raw"
MODEL_PATH="../assets/models/align150-fp32"
OUTPUT_FILE_PATH = "../assets/images/output.raw"
...
def crop(image, pts=np.array([[141, 468], [409, 468], [409, 736], [141, 736]]), shift=0, scale=1.3, rotate=0, res=(128, 128)):
    ...
    cv2.imwrite(IMAGE_CROP_PATH, image_mini)
    return image_mini, M

def read_image(image_path=IMAGE_FILE_PATH, imgnp_path=IMAGE_DATA_PATH, imgnp_mark=IMAGE_MARK_PATH):
    ...
    with open(imgnp_path, "wb") as f:
        img_np.tofile(f)
    with open(imgnp_mark, "wb") as f:
        M.tofile(f) 
    return img_np, M

def read_raw_file(imgnp_path=IMAGE_DATA_PATH, imgnp_mark=IMAGE_MARK_PATH):
    img_np = np.fromfile(imgnp_path, dtype=np.float32)
    img_np.resize(1, 3, 128, 128)
    print("img_np.shape=", img_np.shape)
    print("img_np.shape=", img_np.dtype)
    M = np.fromfile(imgnp_mark, dtype=np.float64)
    M.resize(2, 3)
    print("M.shape=", M.shape)
    print("M.shape=", M.dtype)
    return img_np, M
...
def lite_ouptut(output_path=OUTPUT_FILE_PATH, imgnp_mark=IMAGE_MARK_PATH, image_path=IMAGE_FILE_PATH):
...
if __name__ == '__main__':
    ...
    # lite_ouptut(output_path='../assets/images/output.raw')