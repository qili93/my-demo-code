...
conv_filter_data = np.arange(-20, filter_size-20, dtype=np.float32).reshape((output_channel, 1, kernel_h, kernel_w)) 
print("Filter Data = [{}, {}, {}, {}]".format(output_channel, 1, kernel_h, kernel_w))
for oc in range(output_channel):
    for kh in range(kernel_h):
        print("[ {} ]".format(" ".join(str(float_formatter(v)) for v in conv_filter_data[oc,0,kh,:])))
    print("")
print("-----------------------")

conv_bias_data = np.zeros([output_channel], dtype=np.float32)
print("Bias Data = {}".format(output_channel))
print("[ {} ]".format(" ".join(str(float_formatter(v)) for v in conv_bias_data)))
print("")
...
with fluid.program_guard(main_prog, startup_prog):
    ...
    # conv
    conv_weight_attr = fluid.ParamAttr(name="conv_weight_1", initializer=fluid.initializer.NumpyArrayInitializer(conv_filter_data))
    conv_bias_attr = fluid.ParamAttr(name="conv_bias_1", initializer=fluid.initializer.NumpyArrayInitializer(conv_bias_data))
    ...
    #                                       is_test=True)
    # relu
    relu = paddle.fluid.layers.relu(conv)
...
fluid.io.save_inference_model(dirname=save_dirname, feeded_var_names=['img'],
                target_vars=[relu], executor=exe, main_program=main_prog)
