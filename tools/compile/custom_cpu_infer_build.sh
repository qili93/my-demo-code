...
BUILD_DIR="/workspace/Paddle/build_custom_cpu_infer"
if [ ! -d ${BUILD_DIR} ];then
    mkdir -p ${BUILD_DIR}
fi
...
cmake .. -DPY_VERSION=3.7 -DON_INFER=ON -DWITH_DISTRIBUTE=OFF -DWITH_ARM=${WITH_ARM} \
         -DWITH_GPU=OFF -DWITH_TESTING=OFF -DWITH_CUSTOM_DEVICE=ON \
         -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_VERBOSE_MAKEFILE=ON; cmake_error=$?
